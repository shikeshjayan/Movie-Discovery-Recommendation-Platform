...
const SearchResult = ({ movies, activeIndex, setActiveIndex, onClose }) => {
  ...
  const handleItemClick = (item) => {
    ...
    const route = item.media_type === "tv" ? `/tvshow/${item.id}` : `/movie/${item.id}`;
    navigate(route);
    if (onClose) onClose();
  };

  const renderPersonItem = (item, index) => {
    const knownFor = item.known_for?.map((k) => k.title || k.name).slice(0, 3).join(", ") || "Unknown";
    return (
      <div
        key={item.id}
      ...
      >
        <img
          src={item.profile_path ? `https://image.tmdb.org/t/p/w92${item.profile_path}` : "https://via.placeholder.com/45x68?text=No+Img"}
          alt={item.name}
          className="w-11 h-16 object-cover rounded shadow-sm"
        />
        <div className="flex-1 text-left">
          <h4 className={`text-sm font-semibold line-clamp-1 ${theme === "dark" ? "text-[#FAFAFA]" : "text-[#312F2C]"}`}>
            {item.name}
          </h4>
          <div className={`flex items-center gap-2 text-xs text-gray-400 mt-1 ${theme === "dark" ? "text-[#FAFAFA]" : "text-[#312F2C]"}`}>
            <span className="uppercase px-1 rounded text-[10px] bg-blue-500/30 text-blue-400">Person</span>
